...
class encrypt:
    ...
    def extend(self,ex):
        extend_table = [ex[12-1],  ex[1-1],  ex[2-1],  ex[3-1],  ex[4-1], ex[5-1],
    ...
    def encryption(self):
        for x in range(6):
            ...
            print("XOR extended right & key:", new)
            ...
            for i in range (0,18,6):
                temp = sbox[y][int(''.join(new[i]+new[i+5]),2)][int(''.join(new[i+1:i+5]),2)]
                if y < 3: 
                    y+=1
                temp=bin(temp)[2:].zfill(4)
                temp1.append([int(j) for j in str(temp)])
             
            array1=[]
            for i in range(3):
                for j in range(4):
                    array1.append(temp1[i][j])
            print("S BOX output ",  temp1)
